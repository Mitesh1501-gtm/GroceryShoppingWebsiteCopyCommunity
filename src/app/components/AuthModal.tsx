...
export function AuthModal({ onClose, onAuthSuccess, initialTab = 'login' }: AuthModalProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const name = user.user_metadata?.full_name
      || loginForm.email.split('@')[0].replace(/[._-]/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    onAuthSuccess({ id: user.id, name, email: user.email! });
  };
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const user = data.user!;
    onAuthSuccess({ id: user.id, name: signupForm.name, email: signupForm.email });
  };
